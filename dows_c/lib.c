// lib.c : Send data to a WSS WebSocket in C.
//
...
int onmessage(wsclient *c, wsclient_message *msg) {
	fprintf(stdout, "onmessage: (%llu): %s\n", msg->payload_len, msg->payload);
	return 0;
}

int onopen(wsclient *c) {
	fprintf(stdout, "onopen called: %d\n", c->sockfd);
	libwsclient_send(c, "{\"action\":\"sendmessage\", \"data\":\"hello Ubuntu C\"}");
	return 0;
}
...
void DoWSS()
{
	...
	wsclient *client = libwsclient_new("wss://WSL_ENDPOINT");
	if(!client) {
